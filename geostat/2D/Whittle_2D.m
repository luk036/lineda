...
var_G=0;   % actual variance of inter-die global variation component
...
P=zeros(nchip,nw);
% modify periodogram by window function
% window1=hann(ny);
% window1=repmat(window1,1,nx);
% window2=hann(nx);
% window2=repmat(window2,1,ny);
% window1=ones(ny,nx);
% window2=ones(nx,ny);
% for i=1:nchip
%     X=fft2(reshape(M(i,:),ny,nx));
% %     X=fft(window2.*fft(window1.*reshape(M(i,:),ny,nx)).').';
...
% plot for test
% figure;
% surf(wx,wy,reshape([0 P(1,:)],nwy,nwx));
...
[par,loglik]=fmincon(@obj_Whittle,par0,A,b,[],[],lb,ub,[],options,...
    omega_x2,omega_y2,P);
...
var_s=pi*sum( (alpha_fit^2+omega_x2+omega_y2).^(nu_fit+1).*sum(P-tau2_fit/(4*pi*pi)) ) ...
    /(nchip*nsite*alpha_fit^(2*nu_fit)*nu_fit);

disp('Extracted intrachip spatial variance Var[s]:'); disp(var_s);
disp('Extracted random residual variance Var[r]:'); disp(tau2_fit);
