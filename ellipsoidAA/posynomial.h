...
public:
  /** Constructor */
  explicit posynomial(size_t n, size_t N) : _M(N, monomial<_Tp>(n)) {}
  ...
  /** Add */
  _Self operator+(const monomial<_Tp> &m) const { return _Self(*this) += m; }

  /** Add */
  _Self operator+(const _Self &P) const { return _Self(*this) += P; }
  ...
  template <typename _Up> _Tp operator()(const _Up &y) const {
