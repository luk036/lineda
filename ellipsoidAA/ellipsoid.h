...
template <typename _Tp> class bounding {
public:
  bounding(_Tp &x, double R) : _x(x), _U(x + R), _L(x - R) { assert(R > 0); }
...
/** Enclosing ellipsoid */
template <class Vector> class ellipsoid {
...
public:
  ...
  template <class Vec> double calc_error(const Vec &g) {
    // xxx double dot = 0.;
    // xxx for (size_t i=0; i<_n; ++i) {
    // xxx   _Aeg[i] = 0.;
    // xxx   for (size_t j=0; j<_n; ++j) {
    // xxx 	_Aeg[i] += _Ae[i][j] * g[j];
    // xxx   }
    // xxx   dot += g[i]*_Aeg[i];
    // xxx }
