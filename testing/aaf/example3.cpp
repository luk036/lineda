/*
 * example3.cpp -- This example generates datas and commands of the IA and the
 * AA model
...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    cout << "Usage: " << argv[0] << " LOWER_BOUND UPPER_BOUND BOXN [function]"
         << endl;
    return 0;
  }
  ...
  cout << "AA: [" << min1 << ":" << max1 << "], " << max1 - min1 << ", "
       << 100 * (area1 / area2) << "% , " << 100 * (ydelta_tot1 / ydelta_tot2)
       << "%" << endl
       << endl;
  cout << "(yrange = [" << min << ":" << max << "])" << endl;

  if (!(fin1 && fin2)) {
    cout << "There are some infinite or NaN values so statistics are not "
            "reliable"
         << endl;
  }
