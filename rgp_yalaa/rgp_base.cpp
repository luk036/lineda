...
template <typename AF> typename AF::base_t max(const AF &af, pmap &pol) {
...
template <typename AF> typename AF::base_t eval(const AF &af, const pmap &pol) {
  typename AF::ac_t ac(af.ac());
  typename AF::base_t res = ac.central();
  for (auto it(ac.begin()); it != ac.end(); ++it) {
...
template <class Arr> void rgp_base::assess(const Arr &x) {
...
template void rgp_base::assess(const std::valarray<double> &x);
