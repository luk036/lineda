function sp = spcvxsd(knots,k,x,y)
%
% spcvxsd: convex fitting with tensor-product B-spline for bivariate
% cases with scattered data
%
% Input:
...
cvx_begin sdp
  variable pp(nn)
  for i=1:length(y)
    Cx = spcol(u,m,x(1,i)); % collocation matrix
    Cy = spcol(v,n,x(2,i));
            ...
            for r=0:1
              for s=0:1
                [m*(m-1)*((p(i,j-k-r) - p(i-1,j-k-r))/(u(i+m-1) - u(i-1)) - ...
                 ...
                 n*m*(p(i-l,j-k) - p(i-l,j-k-1) - p(i-l-1,j-k) + p(i-l-1,j-k-1)) ...
