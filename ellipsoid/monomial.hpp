...
class monomial : boost::operators<monomial<_Tp> >,
                 boost::operators2<monomial<_Tp>, _Tp> {
  typedef std::valarray<double> Vec;
  typedef monomial<_Tp> _Self;

public:
  /** Constructor */
  explicit monomial(size_t n) : _a(0., n), _b(_Tp(0)) {}

  /** Constructor */
  monomial(size_t n, const _Tp &c) : _a(0., n), _b(log(c)) {}

  /** Constructor (for AAF -> double) */
  template <typename _Up> monomial(const monomial<_Up> &p, const _Up &polarity);
  ...
  // xxx /** Multiply */
  // xxx _Self operator*(const _Self& m) const { return _Self(*this) *= m; }
  // xxx
  // xxx /** Divided */
  // xxx _Self operator/(const _Self& m) const { return _Self(*this) /= m; }
  // xxx
  /** Multiply */
  // xxx _Self operator*(const _Tp& c) const { return _Self(*this) *= c; }
  // xxx
  // xxx /** Divide */
  // xxx _Self operator/(const _Tp& c) const { return _Self(*this) /= c; }
  ...
  const Vec &gradient(const std::valarray<_Up> &) const {
    return _a;
  }
...
/** Square root */
template <typename _Tp> inline monomial<_Tp> sqrt(monomial<_Tp> m) {
  m.sqrt();
  return m;
}
...
/** Division */
template <typename _Tp>
inline monomial<_Tp> operator/(const _Tp &c, const monomial<_Tp> &m) {
  return monomial<_Tp>(m._a.size(), c) / m;
}

/** Division */
template <typename _Tp>
inline monomial<_Tp> operator/(double c, const monomial<_Tp> &m) {
  return monomial<_Tp>(m._a.size(), _Tp(c)) / m;
}

// xxx /** Multiplication */
// xxx template <typename _Tp>
// xxx inline monomial<_Tp> operator*(const _Tp& c, const monomial<_Tp>& m)
// xxx {
// xxx   return m * c; // assume commutative
// xxx }
