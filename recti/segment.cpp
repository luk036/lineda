...
template <> const Segment<int> Segment<int>::_origin = segment();

template <> const char *Segment<int>::_svg_style = "fill:red;stroke:black";
...
template <> svgstream &operator<<(svgstream &osvgs, const Segment<int> &l) {
  osvgs << "<line"
        << " x1=\"" << l.start().x() << "\""
        << " y1=\"" << l.start().y() << "\""
        << " x2=\"" << l.end().x() << "\""
